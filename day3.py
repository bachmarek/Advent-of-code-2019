...
def part_one(body):
  ...
  for movement in body:
    ...
    if direction == "R":
      # zapis mezikroku
      for a in range(x + 1, x + distance + 1):
        output.append(positionToString([a, y]))
    elif direction == "L":
      for a in range(x - 1, x - distance - 1, -1):
        output.append(positionToString([a, y]))
    elif direction == "D":
      for b in range(y - 1, y - distance - 1, -1):
        output.append(positionToString([x, b]))
    elif direction == "U":
      for b in range(y + 1, y + distance + 1):
        output.append(positionToString([x, b]))
    else:
      print("Invalid input")
    currentPosition = stringToPosition(output[len(output) - 1])
  return output

path1 = part_one(wire_one)
path2 = part_one(wire_two)
...
for point in intersections:
  ...
  inter_list.append(manhattan)
  closest_intersection = min(inter_list)
print(closest_intersection)

route_list = []
for i in intersections:
  route = path1.index(i) + path2.index(i) +2
  route_list.append(route)

min_route = min(route_list)
print(min_route)